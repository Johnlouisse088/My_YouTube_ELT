...
# Insert one video (row) into either the staging table or the core table.
def insert_rows(cur, conn, schema, row):
    """
    cur     → executes the SQL
    conn    → database connection
    schema  → "staging" or "core"
    row     → video data to insert
    """
    try:
        if schema == "staging":
            video_id = "video_id"
            row_video_data = f"""
                ...
                """
            # first argument what sql should execute
            # second argument (row) contains the actual values that will replace the placeholders in the SQL.
            cur.execute(row_video_data, row)
